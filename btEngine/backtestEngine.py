...
def initialize_DB_connection():
    ...
    dailyPriceDB = MyDBApi_GeoDaily({'user': 'root', 'password': 'trinnacle17',\
        'host': '127.0.0.1', 'database': 'GeoTickersDaily07_17',})
    dailyPriceDB.connect()
    dailyBetaSPYDB = MyDBApi_GeoDaily({'user': 'root', 'password': 'trinnacle17',\
        'host': '127.0.0.1', 'database': 'GeoTickersBetaSPY07_17',})
...
class MyBacktestEngine():
    ...
    def __init__(self):
        ...
        self.badTickers = {} # all tickers without data
    ...
    def run(self, dailyStrategy):
        ...
        for iDate in range(len(self.calendar)):
            ...
            print('Net position: ' + str(self.total_position[iDate]))
    ...
    def place_orders_batch(self, i_date, order_batch, use_minute_data=False):
        ...
        # avoid order conflicts, cancel all orders for today for tickers with 'closs all' overwrite
        tickers_to_close = {}
        for iOrder in order_batch:
            if iOrder['volume'] == 'close all':
                tickers_to_close[iOrder['ticker']] = 1
        ...
        for iOrder in order_batch:
            ...
            # exclude 'close all' tickers for today
            if (iTicker in tickers_to_close) and iOrderAmount != 'close all':
                continue

            # exclude tickers without data
            if iTicker in self.badTickers:
                continue
            ...
            if iOrderAmount == "close all":
                ...
                if (self.cash_position[i_date] > 0):
            ...
            print(str(iOrderAmount) + ' shares of ' + iTicker + ' at ' + str(iCost) + '.')
...
class Strategy():
    ...
    def __init__(self):
        ...
        self.subRevSignals = pd.DataFrame.from_csv("/media/trinnacle/86C6-B046/BTEngineTrinnacle/signals/subRevStrategy/oneday_subRevSignalBT10percent.csv")
        self.preprocess()
...
a = MyBacktestEngine()
a.create_calendar('2015-03-01', '2017-11-30')
a.load_daily_close()
